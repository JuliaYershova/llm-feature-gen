...
Use `max_completion_tokens` to set the completion limit. `max_tokens` remains a backwards-compatible alias, but the two options cannot be passed together. Set `reasoning_effort` only for models that support it.
...
provider = OpenAIProvider(max_completion_tokens=4096, temperature=0.0)
